				...
				<header class="page-header author-header">
					...
					if ( ! empty( $curauth->ID ) ) {
						$alt = sprintf(
							...
							_x( 'Profile picture of %s', 'Avatar alt', 'aveiromais' ),
					...
					if ( ! empty( $curauth->user_url ) || ! empty( $curauth->user_description ) ) {
						...
						<dl>
							<?php if ( ! empty( $curauth->user_url ) ) : ?>
								<dt><?php esc_html_e( 'Website', 'aveiromais' ); ?></dt>
								<dd>
					...
					if ( have_posts() ) {
						printf(
							/* translators: %s: author name */
							'<h2>' . esc_html__( 'Posts by %s', 'aveiromais' ) . '</h2>',
							$curauth->display_name
						);
					}
					?>
